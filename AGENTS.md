...
1. **先调研**：联网查官方文档、大厂工程实践（Microsoft/NVIDIA/Google 等技术博客）、开源项目同类实现。面试考察的是对标准解法的认知，不是自创能力。
...
每次踩坑（报错排查、静默劣化、性能意外）必须在 `docs/pitfalls.md` 登记：现象 → 根因 → 修复 → 一句话规律。**登记后同类别错误不得再犯**——这是比规范本身更重要的资产（面试素材）。
