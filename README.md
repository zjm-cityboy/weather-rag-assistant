...
一句话总结（面试口径）：**清洗让知识库干净（噪音块 −100%）、HNSW 让检索快 28 倍、双语 RRF 让英文教材可被中文问题召回（0→2 席）、混合检索+精排让 top-1 准确率提升 66.7%、知识图谱让关系类问题检索快 ~35 倍、关思考让首字节快 74 倍、RAGAS 评估系统忠实度 0.877——每个数字都有实验脚本和计算公式可复现。**
